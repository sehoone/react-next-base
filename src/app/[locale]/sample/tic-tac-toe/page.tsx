...
const deriveWinner = (board: PlayerSymbol[][], playerName: PlayerName) => {
  ...
  for (const combination of WINNING_COMBINATIONS) {
    const firstSquareSymbol = combination[0]?.row !== undefined && combination[0]?.cell !== undefined
      ? board[combination[0].row]?.[combination[0].cell]
      : null;
    const secondSquareSymbol = combination[1]?.row !== undefined && combination[1]?.cell !== undefined
      ? board[combination[1].row]?.[combination[1].cell]
      : null;
    const thirdSquareSymbol = combination[2]?.row !== undefined && combination[2]?.cell !== undefined
      ? board[combination[2].row]?.[combination[2].cell]
      : null;
...
const deriveBoard = (gameTurn: Turn[]) => {
  ...
  for (const turn of gameTurn) {
    ...
    if (board[row] && board[row][cell] !== undefined) {
